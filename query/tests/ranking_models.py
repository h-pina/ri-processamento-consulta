...
class RankingModelTest(unittest.TestCase):
    def setUp(self):
        self.arr_indexes = [
            {
                "a": [TermOccurrence(1, 1, 1), TermOccurrence(3, 1, 1)],
                "feia": [TermOccurrence(2, 5, 1), TermOccurrence(4, 5, 1)],
                "casa": [
        ...
        self.arr_queries_per_idx = [
            [
                {
                    "casa": TermOccurrence(None, 2, 1),
                    "feia": TermOccurrence(None, 5, 1),
                },
                {"crocodilo": TermOccurrence(None, 2, 1)},
            ],
            [{"new": TermOccurrence(None, 1, 1), "times": TermOccurrence(None, 6, 1)}],
        ]
    ...
    def test_precomputed_vals(self):
        ...
        for doc_id, norma_esperada in norma_esperada_per_doc.items():
            self.assertAlmostEqual(
                norma_esperada,
                precomp.document_norm[doc_id],
                places=2,
                msg=f"Norma inesperada do documento {doc_id}",
            )
    ...
    def test_boolean_model(self):
        ...
        for idx, map_index in enumerate(self.arr_indexes):
            for query_position, map_query in enumerate(self.arr_queries_per_idx[idx]):
                ...
                lst_response, _ = model_and.get_ordered_docs(
                    map_query, map_index_for_query
                )
                set_response = set(lst_response)
                self.assertSetEqual(
                    set_response,
                    arr_set_esperado_and_per_query[idx][query_position],
                ...
                lst_response, _ = model_or.get_ordered_docs(
                    map_query, map_index_for_query
                )
                set_response = set(lst_response)
                self.assertSetEqual(
                    set_response,
                    arr_set_esperado_or_per_query[idx][query_position],
    ...
    def test_vector_model(self):
        ...
        peso_por_doc_esperado_per_query = [
            [{1: 0.12, 2: 1.01, 3: None, 4: 0.9}, {}],
            [{1: 0.709, 2: 0.19, 3: 0.15}],
        ]
        arr_norm_por_index = [
            {1: 1.44, 2: 1.16, 3: 2.08, 4: 1.3},
            {1: 1.93, 2: 1.78, 3: 2.31},
        ]
        for idx, map_index in enumerate(self.arr_indexes):
            ...
            for query_position, map_query in enumerate(self.arr_queries_per_idx[idx]):
                ...
                map_index_for_query = self.obtem_index_for_query(map_query, map_index)
                lst_response, doc_weights = vector_model.get_ordered_docs(
                    map_query, map_index_for_query
                )
                self.assertListEqual(
                    lst_response,
                    arr_lst_esperado_per_query[idx][query_position],
                    msg=f"resposta não esperada para a consulta  {query_position} indice {idx}",
                )

                for doc_id, peso in peso_por_doc_esperado_per_query[idx][
                    query_position
                ].items():
                    if doc_id not in doc_weights:
                        self.assertTrue(
                            peso is None,
                            f"O documento {doc_id} deveria ser recuperado da consulta {query_position} indice {idx}",
                        )
                    if peso is None:
                        self.assertTrue(
                            doc_id not in doc_weights,
                            f"O documento {doc_id} não deveria ser recuperado da consulta {query_position} indice {idx}",
                        )
                    else:
                        self.assertAlmostEqual(
                            peso,
                            doc_weights[doc_id],
                            places=2,
                            msg=f"Peso inesperado do documento {doc_id} consulta {query_position} índice {idx}. Peso calculado:{doc_weights[doc_id]} deveria ser: {peso}",
                        )
